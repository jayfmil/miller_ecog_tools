...
class GroupSME(Group):
    ...
    def __init__(self, analysis='sme_enc_nav', subject_settings='sme_nav', open_pool=False, n_jobs=100, **kwargs):
    ...
    def plot_tstat_sme(self, res_key='ts_sme', region=None):
        ...
        if region is None:
            ts = np.stack([x.res[res_key].mean(axis=1) for x in self.subject_objs], axis=0)
            region = 'All'
        else:
