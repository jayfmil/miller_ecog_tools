...
class SubjectSMETime(SubjectAnalysis):
    ...
    def analysis(self):
        ...
        rec_mean = np.nanmean(X[recalled==1], axis=0).reshape(self.subject_data.shape[1:])
        nrec_mean = np.nanmean(X[recalled==0], axis=0).reshape(self.subject_data.shape[1:])
        ...
        ts, ps, = ttest_ind(X[recalled==1], X[recalled==0])
        ...
        lfa_ts, lfa_ps = ttest_ind(lfa_pow[recalled==1], lfa_pow[recalled==0])

        # high freq
        hfa_inds = self.freqs >= 60
        hfa_pow = np.mean(X.reshape(self.subject_data.shape)[:, hfa_inds, :, :], axis=1)
        hfa_ts, hfa_ps = ttest_ind(hfa_pow[recalled==1], hfa_pow[recalled==0])
