...
class SubjectBRINoveltySpikePhaseWithShuffleWaveletAnalysis(SubjectAnalysisBase, SubjectBRIData):
    """

    """

    def __init__(self, task=None, subject=None, montage=0):
        super(SubjectBRINoveltySpikePhaseWithShuffleWaveletAnalysis, self).__init__(task=task, subject=subject, montage=montage)

        # this needs to be an event-locked analyses
        self.do_event_locked = True

        # also compute power/phase at frequencies
        ...
        self.res_str = 'wavelet_novelty_phase_stats_with_shuff.hdf5'
        ...
        with Parallel(n_jobs=int(NUM_CORES/2) if NUM_CORES != 1 else 1,  verbose=5) as parallel:
            ...
            for session_name, session_grp in self.subject_data.items():
                ...
                for channel_num, channel_grp in tqdm(session_grp.items()):
                    ...
                    phase_data = compute_phase(eeg_channel,
                                               self.power_freqs,
                                               self.buffer,
                                               None)
...
def _power_fr_by_event_cond(spike_counts, power_data, phase_bin_start, phase_bin_stop, events, h_file):

    time_ind = (power_data.time.data > phase_bin_start) & (power_data.time.data < phase_bin_stop)
    is_novel = events.isFirst.values

    # compute (zscored) power for each condition
    zpower_data = zscore(power_data[:, time_ind], axis=0)
    pow_novel = zpower_data[is_novel].mean(axis=1)
    pow_rep = zpower_data[~is_novel].mean(axis=1)
...
def compute_phase(eeg, freqs, buffer_len, parallel=None):
    ...
    f = compute_wavelet_at_single_freq
    if parallel is None:
        phase_data = []
        for freq in freqs:
            phases = f(eeg, freq, buffer_len)
...
def compute_phase_stats_with_shuffle(events, spike_rel_times, phase_data, phase_bin_start,
    ...
    spike_phases = _compute_spike_phase_by_freq(spike_rel_times_tmp,
                                                        phase_bin_start,
                                                        phase_bin_stop,
                                                        phase_data,
                                                        events)
    ...
    novel_phases, rep_phases = _bin_phases_into_cond(spike_phases, events)
...
def run_phase_stats_with_shuffle(events, spike_rel_times, phase_data, phase_bin_start,
    ...
    if ~np.any(np.isnan(stats_real)):
        ...
        if isinstance(parallel, Parallel):
            shuff_res = parallel((delayed(f)(events, spike_rel_times, phase_data, phase_bin_start,
                                             phase_bin_stop, True) for _ in range(num_perms)))
        else:
            shuff_res = []
            for _ in range(num_perms):
                shuff_res.append(f(events, spike_rel_times, phase_data, phase_bin_start,
